...
const ChessNews: FC<Props> = ({ pageConfigData, postCategoryData, loading, onUpdatePageConfig }) => {
  ...
  const handleSubmit = async () => {
    ...
    onUpdatePageConfig(
      JSON.stringify(
        pageConfigData
          ? { ...pageConfigData, chessNews: requestBody }
          : {
              chessNews: requestBody
            }
      )
    )
  }
  ...
  useEffect(() => {
    ...
    form.setFieldsValue({
      title: pageConfigData.chessNews?.title ?? '',
      postCategoryId: pageConfigData.chessNews?.postCategorySlug
        ? postCategoryData?.find((item) => item.slug === pageConfigData.chessNews.postCategorySlug)?.id
        : null,
      quantity: pageConfigData.chessNews?.quantity ?? 2
    })
