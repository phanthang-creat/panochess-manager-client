...
import { useGetOrderByIdQuery, usePatchOrderMutation } from '~/stores/server/order/orderStore'
...
export const OrderEditPage = () => {
    ...
    const getOrderByIdQuery = useGetOrderByIdQuery({ id: id ?? ''})

    const patchMutation = usePatchOrderMutation()
    ...
    // }, [form, getBranchesQuery.data, getOrderStatusesQuery.data])

    useEffect(() => {
        if (getOrderByIdQuery.data) {
            const order = getOrderByIdQuery.data
